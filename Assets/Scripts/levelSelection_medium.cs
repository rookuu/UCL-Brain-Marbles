...
public class levelSelection_medium : MonoBehaviour {
	...
	void Awake () {
		...
		for (int i = 1; i <= numberOfLevels; i++) {
			_cmd.CommandText = "SELECT * FROM `levelsessions` WHERE `userid`=@userid AND `stageid`=" + i+8 + ";";
		...
		if (lastCompleted != numberOfLevels && lastCompleted != 0) {
